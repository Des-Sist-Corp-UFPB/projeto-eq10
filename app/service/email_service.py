"""Outbound email — ported from src/auth/email_service.py. Fake/local mode is the
default; real SMTP sending only happens when EMAIL_ENABLED=true and
EMAIL_PROVIDER=smtp (env var names as used by the legacy code and .env.example,
not the SMTP_HOST/PORT/USER/PASSWORD/FROM names sketched in the migration prompt —
see docs/claude-migration.md).
"""
...
EMAIL_PROVIDER_NOT_IMPLEMENTED_MESSAGE = "Envio real por este provedor ainda nao foi implementado."
...
def _normalize_provider(value: str | None) -> str:
    return (value or "fake").strip().lower() or "fake"
...
class EmailService:
    """Fake/local por padrao. Envia SMTP real somente com EMAIL_ENABLED=true e
    EMAIL_PROVIDER=smtp.
    """
    ...
    def send_email(
        ...
        *,
        ...
        if not (to or "").strip() or "@" not in to:
            return self._result(
                success=False, sent=False, mode="validation",
                message="Destinatario de e-mail invalido.", error_code="invalid_recipient",
        ...
        if not (subject or "").strip():
            return self._result(
                success=False, sent=False, mode="validation",
                message="Assunto de e-mail invalido.", error_code="invalid_subject",
                message_type=message_type, recipient=recipient,
            )

        if not self.config.enabled or provider in FAKE_PROVIDERS:
            return self._result(
                success=True, sent=False, mode="fake",
                message=EMAIL_DISABLED_MESSAGE, error_code="email_disabled",
                message_type=message_type, recipient=recipient,
            )

        if provider not in SUPPORTED_PROVIDERS:
            return self._result(
                success=False, sent=False, mode="configuration",
                message=EMAIL_UNSUPPORTED_PROVIDER_MESSAGE, error_code="unsupported_provider",
                message_type=message_type, recipient=recipient,
            )
        ...
        if missing_error:
            return self._result(
                success=False, sent=False, mode="configuration",
                message=EMAIL_CONFIG_INCOMPLETE_MESSAGE, error_code=missing_error,
                message_type=message_type, recipient=recipient,
            )
        ...
        return self._result(
            success=False, sent=False, mode="not_implemented",
            message=EMAIL_PROVIDER_NOT_IMPLEMENTED_MESSAGE, error_code="provider_not_implemented",
            message_type=message_type, recipient=recipient,
        )

    def send_password_reset_email(self, to: str, reset_url: str, *, expires_in_minutes: int = 60) -> EmailSendResult:
        ...
        return self.send_email(to, "Redefinicao de senha", body_text, body_html, message_type="password_reset")
    ...
    def _send_smtp_email(
        ...
        message_type: str, recipient: str,
    ) -> EmailSendResult:
        ...
        try:
            ...
            logger.warning("Email service SMTP auth failed | type=%s | recipient=%s", type(exc).__name__, recipient)
            return self._result(
                success=False, sent=False, mode="smtp",
                message=EMAIL_SMTP_AUTH_FAILED_MESSAGE, error_code="smtp_auth_failed",
                message_type=message_type, recipient=recipient,
            )
        except (smtplib.SMTPException, OSError) as exc:
            logger.warning("Email service SMTP send failed | type=%s | recipient=%s", type(exc).__name__, recipient)
            return self._result(
                success=False, sent=False, mode="smtp",
                message=EMAIL_SMTP_SEND_FAILED_MESSAGE, error_code="smtp_send_failed",
                message_type=message_type, recipient=recipient,
            )

        return self._result(
            success=True, sent=True, mode="smtp",
            message=EMAIL_SMTP_SENT_MESSAGE, error_code=None,
            message_type=message_type, recipient=recipient,
        )

    def _result(
        self, *, success: bool, sent: bool, mode: str, message: str,
        error_code: str | None, message_type: str, recipient: str,
    ) -> EmailSendResult:
        result = EmailSendResult(
            success=success, sent=sent, provider=self.config.provider, mode=mode,
            message=message, error_code=error_code, message_type=message_type, recipient=recipient,
        )
        logger.info(
            "Email service | provider=%s | mode=%s | type=%s | recipient=%s | sent=%s | success=%s | code=%s",
            result.provider, result.mode, result.message_type, result.recipient,
            result.sent, result.success, result.error_code or "none",
        )
        return result
