...
class TestDockerChatConfig(unittest.TestCase):
    ...
    def test_readiness_server_is_internal_and_supervised(self):
        ...
        self.assertIn('write_pid_file "$READINESS_PID_FILE" "$READINESS_PID"', start_source)
        self.assertIn('write_pid_file "$STREAMLIT_PID_FILE" "$STREAMLIT_PID"', start_source)
        self.assertIn('write_pid_file "$NGINX_PID_FILE" "$NGINX_PID"', start_source)
        self.assertIn('READINESS_PID_FILE="/tmp/eq10-readiness.pid"', start_source)
        self.assertIn('STREAMLIT_PID_FILE="/tmp/eq10-streamlit.pid"', start_source)
        self.assertIn('NGINX_PID_FILE="/tmp/eq10-nginx.pid"', start_source)
        self.assertIn("printf '%s\\n' \"$pid\" > \"$pid_file\"", start_source)
        self.assertIn("remove_pid_files", start_source)
