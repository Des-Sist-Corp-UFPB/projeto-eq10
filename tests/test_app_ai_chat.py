...
ROLES_PATH = Path("src/auth/roles.py")
SESSION_PATH = Path("src/auth/session.py")
...
class TestAppAiChat(unittest.TestCase):
    ...
    def test_app_usa_sidebar_como_navegacao_unica(self):
        ...
        self.assertIn(".st-key-sidebar-nav-auditoria", app_source)
        ...
        self.assertIn('"auditoria": ADMIN_PAGE', sidebar_source)
        self.assertIn('"uso-restrito": ADMIN_PAGE', sidebar_source)
        self.assertIn("show_admin = can_view_audit_log(user)", sidebar_source)
        self.assertIn('st.button("Auditoria"', sidebar_source)
    ...
    def test_admin_auditoria_tem_guarda_de_permissao_no_sidebar_e_rota(self):
        app_source = APP_PATH.read_text(encoding="utf-8")
        sidebar_source = SIDEBAR_PATH.read_text(encoding="utf-8")
        ...
        self.assertIn("from src.auth.roles import can_view_audit_log", app_source)
        self.assertIn("def _resolve_authorized_page", app_source)
        self.assertIn("current_page = _resolve_authorized_page(get_current_page())", app_source)
        self.assertIn("if can_view_audit_log(user):", app_source)
        self.assertIn("st.session_state.admin_access_feedback", app_source)
        self.assertIn("set_current_page(DEFAULT_PAGE)", app_source)
        self.assertIn("Voce nao tem permissao para acessar esta pagina.", app_source)
        self.assertIn("render_sidebar(current_page)", app_source)
        self.assertIn("elif current_page == ADMIN_PAGE:", app_source)
        self.assertIn("render_admin_page()", app_source)

        self.assertIn("show_admin = can_view_audit_log(user)", sidebar_source)
        self.assertIn("if show_admin:", sidebar_source)
        self.assertIn('st.button("Auditoria"', sidebar_source)
        self.assertIn("ADMIN_PAGE = \"Auditoria\"", sidebar_source)
        self.assertNotIn("<nav", sidebar_source)
        self.assertNotIn("</nav>", sidebar_source)

        self.assertIn("ROLE_ADMIN = \"admin\"", roles_source)
        self.assertIn("ROLE_SUPER_ADMIN = \"super_admin\"", roles_source)
        self.assertIn("def is_admin", roles_source)
        self.assertIn("{ROLE_ADMIN, ROLE_SUPER_ADMIN}", roles_source)
        self.assertIn('_user_value(user, "can_view_audit", False)', roles_source)
        self.assertIn('"can_view_audit"', session_source)
        self.assertIn("if not user or not can_view_audit_log(user):", admin_source)
    ...
    def test_inputs_e_acoes_do_modal_tem_estilo_polido(self):
        ...
        self.assertIn("AUTH_MODAL_PROCESSING_STATE_KEYS", auth_source)
        self.assertIn("def set_modal_feedback", auth_source)
        self.assertIn("def _clear_modal_feedback", auth_source)
        self.assertIn("def clear_auth_processing_state", auth_source)
        ...
        self.assertIn("def _finish_modal_action_feedback", auth_source)
        ...
        self.assertIn("target_state.pop(key, None)", auth_source)
        self.assertIn("clear_auth_processing_state(st.session_state)", auth_source)
        ...
        self.assertIn("_finish_modal_action_feedback(exc.public_message, \"error\")", auth_source)
        self.assertIn("_finish_modal_action_feedback(AUTH_UNAVAILABLE_MESSAGE, \"error\")", auth_source)
        self.assertIn("with st.spinner(message):", auth_source)
        self.assertIn("finally:", auth_source)
        self.assertIn("_finish_modal_action_feedback(_first_error_message(field_errors), \"error\")", auth_source)
        self.assertIn("set_modal_feedback(st.session_state, message, kind)", auth_source)
        self.assertIn("Ocorreu um erro. Tente novamente mais tarde.", auth_source)
